...
reads as a designed page you can type into. The same file presents as a deck. The
`README.md` one folder up is this notebook exported for GitHub, which renders it under
the lecture folder's file list; `scripts/readme.py` writes it, and it is committed
because GitHub cannot run the notebook.
...
scripts/make.sh      counts, style cell, execute, trust, export, README. --seed
                     rebuilds doc.ipynb from the seed first and erases hand edits.
scripts/readme.py    ../README.md and assets/charts/*.svg from doc.ipynb, with the
                     HTML GitHub strips (classes, data: images) rewritten to tags it keeps
...
../README.md         the GitHub view. Generated and committed; never edit it by hand.
```
...
```sh
bash scripts/make.sh   # recount, re-execute the charts, re-export, rewrite ../README.md
```
...
the deck, and `scripts/readme.py` from the README.

...
Built to outlive the course it was drafted for: a reference anyone in the field can
use, not a syllabus.

**https://yayuanli-org.github.io/awesome-physical-ai/**

The site is one self-contained page. To work on it offline, clone and build from
this folder:
...
Both HTML files are generated. Edit `data/` or `src/`, never either page. The
workflows that validate and publish live at the repo root, in `.github/workflows/`.
...
```sh
cd paper-hub && python3 serve.py    # http://127.0.0.1:8777/review.html
```
